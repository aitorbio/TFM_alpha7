"""
admet_import_results.py — Módulo 7: Consolidación ADMET y priorización final
=============================================================================
Importa los resultados de predicción ADMET descargados manualmente de tres
plataformas web de acceso libre y genera la priorización final de candidatos.

Las plataformas utilizadas son:
  • SwissADME  (https://www.swissadme.ch)   — BBB, P-gp, logP, Lipinski
  • pkCSM      (https://biosig.lab.uq.edu.au/pkcsm) — logBB, hERG, t½, DILI, AMES
  • ADMETlab 2.0/3.0 (https://admetmesh.scbdd.com) — DILI, AMES, hERG, BBB, BCRP

El flujo de trabajo manual previo a este script (§3.8 del TFM) consiste en:
  1. Exportar los SMILES de hits_top5pct.csv
  2. Subirlos a cada plataforma web
  3. Descargar los CSV resultantes a admet_inputs/

Los criterios de aceptación aplicados son (§3.8.2):
  • CNS MPO ≥ 4.0 (calculado con RDKit en el Módulo 1)
  • BBB permeant (logBB > 0 según pkCSM, o "Yes" en SwissADME)
  • No sustrato de P-gp (eflujo activo reduce exposición cerebral)
  • No inhibidor de hERG (riesgo de cardiotoxicidad; pIC50 < 6.0)
  • Estabilidad microsomal aceptable (t½ > 30 min, o clearance < 10 L/h/kg)
  • No hepatotóxico (DILI Low en ADMETlab o pkCSM)
  • No mutagénico en test de Ames (AMES < 0.5 en ADMETlab)

Los criterios inciertos (None, datos no disponibles) no producen rechazo;
se marcan como "inciertos" para revisión experimental posterior.

La consolidación utiliza voto por mayoría simple entre plataformas.
La priorización final se calcula como:
  0.40 · S_dock + 0.35 · S_IFP + 0.25 · S_CNS
...
Ficheros esperados en --input:
    ...
    02_admetlab_results.csv    (exportado desde ADMETlab 2.0/3.0)
...
def _col(df: pd.DataFrame, *candidates: str) -> str | None:
    """
    Devuelve el primer nombre de columna que existe en df (insensible a mayúsculas).
    Se utiliza para manejar las diferencias de nomenclatura entre las distintas
    versiones y exportaciones de las plataformas ADMET web, que no son consistentes.
    Devuelve None si ningún candidato está presente en el DataFrame.
    """
...
def _to_bool(val) -> bool | None:
    """
    Normaliza los distintos formatos de salida booleana usados por las plataformas ADMET.
    Cada plataforma utiliza convenciones diferentes: SwissADME usa "Yes"/"No",
    pkCSM usa "Substrate"/"Non-substrate", ADMETlab puede usar "Positive"/"Negative"
    o probabilidades numéricas [0–1]. Esta función unifica todos los formatos a
    bool o None (cuando el valor es ambiguo o está ausente).
    """
...
def parse_pkcsm(path: str) -> pd.DataFrame:
    ...
    # Vida media microsomal (t½).
    # pkCSM puede incluir la columna "Half life" directamente o solo
    # "Total Clearance". En ausencia de t½ explícito se usa el clearance
    # como proxy: clearance < 10 L/h/kg equivale aproximadamente a t½ > 30 min,
    # umbral mínimo para candidatos CNS (Di et al., 2013, Drug Metab Dispos).
...
def majority_vote(*values) -> bool | None:
    """
    Voto por mayoría simple entre valores booleanos procedentes de distintas plataformas.
    Ignora los valores None (datos no disponibles en esa plataforma).
    Si hay empate exacto (mismo número de True y False) o todos los valores son None,
    devuelve None para indicar incertidumbre, evitando un rechazo injustificado del
    candidato cuando los predictores no son concordantes.
    """
...
def consolidate(
    ...
    def _join(
        ...
        Estrategia 1 — Por posición (índice de fila):
            Se utiliza cuando el CSV ADMET tiene exactamente el mismo número
            de filas que df_ref. Es el caso habitual: los SMILES se exportan
            en orden y se suben a las webs en ese mismo orden, de modo que
            los resultados devueltos mantienen la misma secuencia.

        Estrategia 2 — Por SMILES canónico (RDKit):
            Se activa cuando los conteos difieren (ej. pkCSM rechazó alguna
            molécula). RDKit canonicaliza ambos SMILES antes del join para
            evitar fallos por representaciones distintas del mismo compuesto.

        Estrategia 3 — Posición parcial (fallback):
            Si las estrategias anteriores fallan, se asignan los primeros
            n_admet registros por posición. Las filas no cubiertas quedan con NaN.
        """
    ...
    for _, row in base.iterrows():
        ...
        # ADMET_pass: evalúa los 7 criterios clave de aceptación (§3.8.2).
        # Los criterios inciertos (None, datos no disponibles en ninguna plataforma)
        # no producen rechazo: se asumen como válidos para la exploración in silico
        # y se registran en ADMET_uncertain_criteria para revisión experimental
        # posterior (in vitro / in vivo). Este comportamiento está justificado en
        # §3.8.3 del TFM: rechazar por ausencia de datos sobreestimaría la tasa
        # de falsos negativos en la fase de priorización computacional.
...
def prioritize(df: pd.DataFrame) -> pd.DataFrame:
    """
    Calcula la puntuación de priorización multi-criterio para cada candidato.
    La fórmula es: S_prio = 0.40·S_dock + 0.35·S_IFP + 0.25·S_CNS

    S_dock se normaliza por min-max entre todos los candidatos (más negativo → 1).
    S_IFP  es la similitud de Jaccard con el IFP de referencia (PNU-120596), en [0, 1].
    S_CNS  es el CNS MPO normalizado por 6 (puntuación máxima posible), en [0, 1].

    Los candidatos se ordenan de mayor a menor S_prio.
    Las ponderaciones difieren de las usadas en composite_score() de docking_pipeline.py
    (0.5/0.3/0.2): aquí el peso del IFP es mayor para equilibrar la priorización
    final con mayor énfasis en la complementariedad de contactos residuales.
    """
    out = df.copy()
...
# ─────────────────────────────────────────────────────────────────────────────
# Figura de resumen ADMET: heatmap de criterios + puntuaciones de priorización
# Panel izquierdo: heatmap de los 7 criterios ADMET (verde=cumple, rojo=no
#   cumple, amarillo=incierto) para los 15 mejores candidatos priorizados.
# Panel derecho: gráfico de barras horizontales con la puntuación de
#   priorización, con línea de corte en ≥ 0.65 (umbral de selección final).
# ─────────────────────────────────────────────────────────────────────────────
