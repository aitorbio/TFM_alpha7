...
====
Construye la biblioteca de cribado virtual a partir de ZINC-22.
El flujo de trabajo sigue la metodología descrita en el §3.4 del TFM:

  1. Adquisición de compuestos desde ZINC-22 (https://zinc22.docking.org),
     aplicando pre-filtros de MW, logP y TPSA directamente en la query
     para reducir el volumen antes del procesamiento local.
  2. Cálculo de propiedades fisicoquímicas y filtrado mediante el Módulo 1
     (mol_properties_rdkit.py): criterios CNS MPO, PAINS y BRENK.
  3. Deduplicación estructural por huella digital Morgan (Tanimoto > 0.85)
     para garantizar diversidad química en la biblioteca resultante.
  4. Generación de figuras diagnósticas: distribuciones de propiedades y
     cascada de filtrado.

La biblioteca resultante (library_filtered.csv) es la entrada del Módulo 3
(docking_pipeline.py) para el cribado virtual con AutoDock Vina.

NOTA: La descarga directa mediante --download-zinc22 utiliza la API REST
de ZINC-22, que en el momento de desarrollo presentaba inestabilidades.
Se recomienda descargar el CSV manualmente desde la interfaz web de ZINC-22
SmallWorld y pasarlo con --input.

Dependencias:
    rdkit >= 2023.09
    pandas, numpy, requests, matplotlib
    mol_properties_rdkit.py  (IMPORTANTE: debe estar en el mismo directorio)

Uso:
# Filtrar un fichero CSV descargado manualmente de ZINC-22
python library_builder.py \\
    --input data/ZINC_raw_download.csv \\
    --output results/library_filtered.csv

# Modo test con los PAMs de referencia integrados en mol_properties_rdkit.py
# Útil para verificar el funcionamiento del pipeline con un conjunto reducido.
python library_builder.py --test

# Descarga automática desde la API de ZINC-22 (experimental, puede no funcionar)
# Por defecto descarga 5000 compuestos con los pre-filtros definidos en ZINC22_QUERY_PARAMS.
...
# Importación de mol_properties_rdkit.py. Es imprescindible que se encuentre
# en el mismo directorio que este script, ya que no es un paquete instalado.
# sys.path.insert garantiza que Python lo encuentre antes que cualquier
# paquete con el mismo nombre en el entorno.
...
try:
    ...
    # RDKit debería estar ya instalado y verificado por verify_installation.py.
    # Si este error aparece, comprobar que el entorno conda correcto está activo.
...
# Descarga desde ZINC-22 mediante la API REST

# URL base de la API de ZINC-22.
ZINC22_BASE = "https://zinc22.docking.org"

# Parámetros de la query a la API de ZINC-22.
# Se aplican pre-filtros de MW, logP y TPSA directamente en el servidor
# para reducir el volumen de datos descargados antes del filtrado local con RDKit.
# Los rangos son consistentes con los criterios CNS MPO del Módulo 1.
ZINC22_QUERY_PARAMS = {
...
# Deduplicación estructural mediante huella digital Morgan (§3.4.4)
...
def deduplicate_by_morgan(
    ...
    """
    Elimina duplicados estructurales mediante huella digital Morgan de forma lineal O(N).
    Para cada compuesto nuevo, se compara su similitud de Tanimoto contra todos los
    compuestos ya retenidos usando el núcleo C++ de RDKit (BulkTanimotoSimilarity).
    Si la similitud máxima supera el umbral (por defecto 0.85), el compuesto se descarta
    y se conserva el de mayor CNS_MPO entre los redundantes.

    Este paso reduce la redundancia química de la biblioteca y mejora la diversidad
    estructural del conjunto enviado a docking, en línea con las mejores prácticas
    de diseño de bibliotecas de cribado virtual.

    Parámetros:
        radius         : radio de la huella Morgan (2 equivale a ECFP4)
        n_bits         : longitud del vector de bits (2048 estándar)
        tanimoto_cutoff: umbral de similitud por encima del cual se considera duplicado
    """
    log.info(
    ...
    for i, row in df.iterrows():
        ...
        # BulkTanimotoSimilarity realiza N comparaciones en una sola llamada C++,
        # siendo sustancialmente más eficiente que un bucle Python equivalente.
...
# Pipeline de filtrado completo (Módulo 2, §3.4)
# Orquesta el procesamiento desde los compuestos crudos hasta la biblioteca
# lista para docking, aplicando en secuencia: cálculo de propiedades,
# filtros fisicoquímicos, alertas estructurales y deduplicación Morgan.
...
def main():
    ...
    if args.test:
        ...
        # NOTA: TEST_COMPOUNDS está definido como (smiles, name) en mol_properties_rdkit.py,
        # pero build_filtered_library espera (smiles, name), por lo que el orden es correcto.
        # En versiones anteriores del código existía una inversión (name, smi) que se corrigió.
