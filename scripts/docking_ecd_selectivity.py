"""
docking_ecd_selectivity.py — Módulo 6: Análisis de selectividad TMD vs ECD
============================================================================
Evalúa la selectividad de sitio de los candidatos seleccionados en el
cribado TMD (Módulo 4) mediante docking contra el sitio ortostérico del
dominio extracelular (ECD) del α7 nAChR.

La selectividad de sitio es una propiedad farmacológica relevante para
distinguir moduladores alostéricos positivos (PAMs) del sitio TMD de
agonistas ortostéricos del ECD. Un compuesto con mayor afinidad por el
TMD que por el ECD (ΔScore = Score_TMD − Score_ECD < 0) es consistente
con el perfil farmacológico de los PAMs tipo II conocidos
(PNU-120596, TQS, GAT107).

Lógica del análisis:
  1. Lee screening_all.csv (candidatos que superaron el cribado TMD)
  2. Localiza los archivos PDBQT ya generados en pdbqt_library/
     (generados por docking_pipeline.py), evitando reconvertir SMILES
  3. Ejecuta docking paralelo contra el receptor ECD con un grid
     centrado en el sitio ortostérico (centroide de epibatidina, EPJ,
     cadena A en PDB 8V82; Burke et al., 2024)
  4. Compara dock_score TMD vs ECD y calcula ΔScore
  5. Genera figura comparativa y CSV de resultados

El grid ECD está centrado en el sitio ortostérico de epibatidina:
  Centro: (150.67, 159.73, 175.91) Å — derivado del centroide de EPJ
  en la cadena A de PDB 8V82 (Burke et al., 2024).
  Este centro es fijo y no debe calcularse desde el ligando TMD, ya que
  ambos sitios están separados ~45 Å.
...
# ─────────────────────────────────────────────────────────────────────────────
# Grid para el sitio ortostérico del ECD (fijo, derivado de coordenadas PDB)
# Las coordenadas del centro son el centroide del ligando epibatidina (EPJ)
# en la cadena A del PDB 8V82 (Burke et al., 2024). Este grid NO debe
# calcularse dinámicamente desde el ligando TMD (PNU-120596), ya que ambos
# sitios están separados ~45 Å y un error en el centro haría que el docking
# no cubriera el sitio ortostérico correctamente.
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# Docking con AutoDock Vina contra el receptor ECD
# run_vina() es funcionalmente idéntico al de docking_pipeline.py pero
# específico para el receptor ECD. Se mantiene como función independiente
# para que este script sea autónomo y no dependa de importar docking_pipeline.
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# Cribado paralelo contra el ECD mediante ProcessPoolExecutor
# Reutiliza los archivos PDBQT ya generados por docking_pipeline.py,
# evitando la costosa reconversión SMILES → 3D → PDBQT para cada candidato.
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# Figura comparativa de selectividad TMD vs ECD (tres paneles)
# Panel 1: Scatter TMD vs ECD — permite identificar visualmente los compuestos
#          con preferencia por el TMD (por debajo de la diagonal y = x).
# Panel 2: Histograma de ΔScore (TMD − ECD) — valores negativos indican mayor
#          afinidad por el TMD (comportamiento esperado para PAMs tipo II).
# Panel 3: Tabla resumen con estadísticas clave y clasificación de selectividad.
# Los finalistas ADMET (candidates_strict_admet_passed.csv) se resaltan en
# el scatter para contextualizar su posición respecto al resto de candidatos.
# ─────────────────────────────────────────────────────────────────────────────
...
def main():
    ...
    # Cargar la lista de finalistas ADMET para resaltarlos en la figura.
    # Este archivo es generado por admet_import_results.py en un paso posterior
    # del pipeline. Si no existe aún (primera ejecución), se asume que ningún
    # compuesto es finalista y el análisis continúa sin errores.
